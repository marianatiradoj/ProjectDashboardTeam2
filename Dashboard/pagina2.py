...
# Page configuration
st.set_page_config(
    page_title="Tendencias Históricas del Crimen",
    layout="wide",
)
apply_theme()

# Header section
st.title("Tendencias Históricas del Crimen (2016–2025)")
st.caption(
    "Este módulo permite identificar patrones temporales, geográficos y tipológicos en la "
    "incidencia delictiva. Los gráficos muestran cómo varían los delitos por hora, día, mes, zona "
    "y categoría, facilitando un análisis integral del comportamiento histórico."
)
st.divider()

# Load central historical dataset (cached in data_loader)
df = load_central_dataset()

# Sidebar filters (shared by all charts on this page)
with st.sidebar:
...
# Right column: weekly time series + monthly composition
with col2:
